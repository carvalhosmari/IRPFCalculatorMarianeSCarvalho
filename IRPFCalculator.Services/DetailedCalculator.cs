...
{
    public class DetailedCalculator : IDetailedCalculator
    {
        public string DetailedTaxCalculation(decimal revenue)
        {
            ...
            decimal tax = revenue * aliquot - deduction;

            return $"Receita anual: R$ {revenue.ToString("F2")}" +
                $"\nAliquota: {(aliquot * 100).ToString("F1")}%" +
                $"\nDedução: R$ {deduction.ToString("F2")}" +
                $"\nTotal a pagar: R$ {tax.ToString("F2")}";
